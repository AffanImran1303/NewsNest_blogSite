...
export default async function BlogPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const post = await prisma.post.findUnique({
    ...
    select: {
      ...
      image: true,
      category: true,
      excerpt: true, // Only works if Step 2 below is successful
    }
  });

  if (!post) return notFound();

  // 2. Fetch Related Posts with explicit selection
  const relatedPosts = await prisma.post.findMany({
    ...
    select: {
      ...
      image: true,
      category: true,
      // Do NOT include 'excerpt' here yet if it's causing the crash
    }
  });
  return (
              ...
              {relatedPosts.map((related) => (
                <Link key={related.id} href={`/blog/${related.slug}`} className="group block">
                  <div className="relative rounded-2xl overflow-hidden mb-4">
                    <img src={related.image} className="w-full aspect-video object-cover transition-transform group-hover:scale-105" />
                    <div className="absolute bottom-3 left-3">
                       <PlayCircle className="text-white fill-[#3D3B8E] w-8 h-8 opacity-90" />
                    </div>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-red-600 mb-1 block">{related.category}</span>
                    <h3 className="font-bold text-lg leading-snug group-hover:underline">{related.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
  ...
  <div className="space-y-3 text-white">
    {/* Apple News Button */}
    <div className="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-zinc-400 transition">
      <span className="text-sm font-bold text-white">Apple News</span>
    </div>

    {/* Google News Button */}
    <div className="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-zinc-400 transition">
      <span className="text-sm font-bold ">Google News</span>
    </div>
  </div>
</div>
          </aside>
